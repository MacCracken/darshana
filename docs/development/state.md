...
| `dist/darshana.cyr` | 909 | Bundled distribution — regenerate via `cyrius distlib`. What consumers `include "lib/darshana.cyr"`. (922 lines on disk; `distlib` reports module-body lines, excluding its 13-line generated header.) |
...
Total source ≈ 909 lines across the three dist modules (grew from ~780
at v0.7.0 with the v0.8.0–v0.9.0 agnos peers). Public fn surface is **29**
...
- **aarch64-Linux `SYS_IOCTL` shadow (latent, unfixed).** `src/termios.cyr:122`
  hardcodes `var SYS_IOCTL = 16` — the x86_64 number — inside the
  arch-blind `#ifdef CYRIUS_TARGET_LINUX` gate, shadowing the stdlib's
  arch-aware definition (`syscalls_aarch64_linux.cyr` has 29). A
  `cyrius build --aarch64` emits `duplicate symbol 'SYS_IOCTL' redefined
  with conflicting value (last definition wins)`, so all five ioctl
  callsites (`tty_raw`, `tty_cooked`, `tty_winsize` ×2) would issue 16
  on aarch64. Whether the ELF aarch64 backend's ESYSXLAT whitelist
  renumbers 16→29 is unverified — the documented ESYSXLAT cases are
  Mach-O and the fsync/fdatasync pair. Pre-dates v0.9.1 (reproduces at
  0.9.0); harmless today since darshana ships x86_64-Linux + agnos and
  has no aarch64 consumer. Fix is likely a one-liner — drop the local
  `var` and use the stdlib's — but it is a behavior change, so it wants
  its own cut, not a toolchain bump.
...
- **Soak-window cuts** (v0.6.0 → v0.9.1) — during the M5 calendar gate:
